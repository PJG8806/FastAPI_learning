...
from query.app.service.meeting_service_mysql import (
...
from query.app.service.meting_service_edgedb import (
...
async def api_create_meeting_mysql() -> CreateMeetingResponse:
    return CreateMeetingResponse(
        url_code=(await service_create_meeting_mysql()).url_code
    )
...
@edgedb_router.patch(
    "/{meeting_url_code}/date_range", description="meeting 의 날짜 range 를 설정합니다."
)
async def api_update_meeting_date_range_edgedb(
    ...
    if update_meeting_date_range_request.exceeds_max_range():
        raise HTTPException(
            status_code=HTTP_422_UNPROCESSABLE_CONTENT,  # HTTP_422_UNPROCESSABLE_ENTITY 가 와야 하는데 지금 없음
...
async def api_update_meeting_date_range_mysql(
    ...
    if update_meeting_date_range_request.exceeds_max_range():
        raise HTTPException(
            status_code=HTTP_422_UNPROCESSABLE_CONTENT,  # 원래 _ENTITY
            detail=f"start {update_meeting_date_range_request.start_date} and end {update_meeting_date_range_request.end_date} should be within {MEETING_DATE_MAX_RANGE.days} days",
        )
...
async def api_update_meeting_title_edgedb(
    ...
    updated = await service_update_meeting_title_edgedb(
        meeting_url_code, title=update_meeting_title_request.title
    )
    if not updated:
        raise HTTPException(
            status_code=HTTP_404_NOT_FOUND,
